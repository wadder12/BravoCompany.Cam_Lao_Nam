...
while {GRLIB_csat_aggressivity > 0.9 && GRLIB_endgame == 0} do {
    ...
    if (!isNil "GRLIB_last_battlegroup_time") then {
        waitUntil {
            sleep 5;
            diag_tickTime > (GRLIB_last_battlegroup_time + (4800 / GRLIB_csat_aggressivity))
        };
    };
    ...
    if (
        ...
        ["", (random 100) < 95] spawn spawn_battlegroup;
        private _waitTime = (1800 + (random 1800)) / (([] call KPLIB_fnc_getOpforFactor) * GRLIB_csat_aggressivity);
        sleep _waitTime;
    };
};