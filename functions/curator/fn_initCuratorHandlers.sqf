...
if (hasInterface) then {
    [true, "KPLIB_zeusAssigned", {        params [            ["_zeus", objNull, [objNull]],
            ["_limited", false, [true]]
        ];
        ...
        // Add a hint to inform players when they become a curator
        if (isPlayer _zeus) then {
            hint format ["You are now a curator. Press '%1' to access Zeus.", toUpper call BIS_fnc_getMissionKey];
        };
    }] call BIS_fnc_addScriptedEventHandler;
};
